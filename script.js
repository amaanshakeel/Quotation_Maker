...
const DRAFT_KEY = "alHashmiQuotationDraft";
...
function getFormData() {
  ...
  return {
    ...
    advanceType: formData.get("advanceType") || "flat",
    includeCarriageNote: formData.has("includeCarriageNote"),
...
function calculateTotals(data) {
  ...
  const advanceValue = Math.max(0, data.advanceAmount);
  const received = data.advanceType === "percent"
    ? (finalAmount * Math.min(100, advanceValue)) / 100
    : advanceValue;
...
function saveDraft(data = getFormData()) {
  ...
  try {
    ...
    // The quotation remains usable if browser storage is unavailable or full.
  }
}
...
function restoreDraft() {
  ...
  setField("guests", draft.guests);
  setField("functionType", draft.functionType);
  setField("functionDate", draft.functionDate);
  setField("contact", draft.contact);
  setField("bookedBy", draft.bookedBy);
  setField("setupReadyTime", draft.setupReadyTime);
  setField("foodTime", draft.foodTime);
  setField("paymentMode", draft.paymentMode);
  setField("advanceAmount", draft.advanceAmount);
  setField("advanceType", draft.advanceType || "flat");
  setCheckedField("includeCarriageNote", draft.includeCarriageNote !== false);
  setField("discount", draft.discount);
  setField("discountType", draft.discountType || "flat");
  itemsBody.innerHTML = "";
  (Array.isArray(draft.items) && draft.items.length ? draft.items : [{}]).forEach((item) => addItemRow(item));
...
function validateQuotation(data) {
  ...
  if (data.advanceType === "percent" && data.advanceAmount > 100) {
    return "Advance percentage cannot be more than 100%.";
  }
...
async function generatePdf(data) {
  ...
  const advanceLabel = data.advanceType === "percent"
    ? `Received (${formatDiscountValue(getEnteredAdvance(data))}%)`
    : "Received";
  const financialRows = [
    ["Total", "", "", formatMoney(data.subtotal)],
    [discountLabel, "", "", formatMoney(data.discount)],
    [advanceLabel, "", "", formatMoney(data.received)],
  ...
  drawNotes(doc, data, margin, tableWidth);
...
function drawInfoRows(doc, data, margin, y, tableWidth, gold) {
  ...
  const rows = [
    ...
    ["Function Date:", formatDate(data.functionDate), getAdvanceLabel(data), formatMoney(data.received)],
...
function drawNotes(doc, data, margin, tableWidth) {
  ...
  doc.setFontSize(9);
  doc.setTextColor(0, 0, 0);
  const notes = data.includeCarriageNote === false ? COMPANY.notes.slice(0, 1) : COMPANY.notes;
...
function buildPreviewHtml(data, totals) {
  ...
  const advanceLabel = data.advanceType === "percent"
    ? `Received (${formatDiscountValue(data.advanceAmount)}%)`
    : "Received";
  const carriageNoteRow = data.includeCarriageNote
  ...
  return `
    <article class="quote-sheet">
      ...
      <table>
        ...
        ${infoRow("Function Date:", formatDate(data.functionDate), getAdvanceLabel(data), formatMoney(totals.received))}
      ...
      <table class="quote-table">
        ...
        <tbody>
          ...
          <tr class="quote-gold"><td>${advanceLabel}</td><td></td><td></td><td class="num">${formatMoney(totals.received)}</td></tr>
...
function loadQuotation(entry) {
  ...
  setField("advanceAmount", getEnteredAdvance(entry));
  setField("advanceType", entry.advanceType || "flat");
  setCheckedField("includeCarriageNote", entry.includeCarriageNote !== false);
...
function setCheckedField(name, checked) {
  const field = quotationForm.elements[name];
...
function getEnteredAdvance(data) {
  if (data.advanceAmount !== undefined) return toNumber(data.advanceAmount);
  if (data.advanceType === "percent" && toNumber(data.finalAmount) > 0) {
    return (toNumber(data.received) / toNumber(data.finalAmount)) * 100;
  }
  return toNumber(data.received);
}

function getAdvanceLabel(data) {
  return data.advanceType === "percent"
    ? `Advance (${formatDiscountValue(getEnteredAdvance(data))}%):`
    : "Advance Amount:";
}
...
function adjustTableSpans() {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const typeSelect = row.querySelector(".item-type");
    const type = typeSelect ? typeSelect.value : "standard";
    row.classList.toggle("category-row", type === "category");
  ...
  groups.forEach(group => {
    if (group.type === "standard") {
      ...
      body.push([
        { 
          content: cat.name, 
          styles: { fillColor: gold, fontStyle: "bold", halign: "center" }
        },
        { 
